...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
    ...
    \item (MD+NP) talk about the loss in 3.8 (old 4.8) \nati{I was very brief. In color for now.}
    ...
    \item (MD) Are you happy with the vertex interpretation in section 3.5 Efficient convolutions? Also do you like the end of 3.4?
    ...
    % \item (done!) (TK) make a new proposition for a plot Figure 6: Two levels of coarsening and pooling
    % \item \todo{Notation GC32 - P256 - FC10 to describe an architecture? Maybe more useful in Experiments where we describe the architecture used for each experiment.}
    % Nati: I did it without this notation. Since we have two architectures, I think it is OK.
\end{itemize}
...
\todo{For a regression task, the most common choice is the $L^2$ norm $\ell(\y) = \|\y-\bar{\y}\|_2^2$, where $\bar{\y}$ is the desired output, as it minimize the mean square error. We emphasis that by sole change of the loss, one can solve regularization instead of classification.}
...
% \todo{How do they handle boundaries?}
% \TK{+1,good question}
% \nati{They are almost not talking about that. For the cubed sphere, the only reference I could found was:
% \emph{"When implementing convolutions and pooling operations for the cubed-sphere grid, one has to be careful in padding each patch with the contents of the four neighbouring patches, in order to preserve the wrapped topology of the sphere (figure 2, right)."}
% Note that for the cubed sphere sampling each pixel has exactly four neighboors which make the problem of border effect handable. As for the equirectangular grid, it is equivalent to a cylinder. So I guess they take the standard image processing border conditions (reflective).
% I do not think we should mention any of that in the paper.
% }
...
% \TK{I would leave out here, as this is a "spoiler" of the results section.}
...
\nati{Why is \citet{boomsma2017spherical} here? I think it can be trivally accelerated.}
...
\TK{This is a strong claim, can we support this?} \nati{It is not that strong as we use "may" right? We should probably soften it a bit. However the main idea is correct. If you compute some statistics on some features, you lose some information and may get worse results. Nevertheless, since we are also using a statistical layer, I would juste remove this.}
...
\TK{why is this section in TODO?\nati{Probably because, we have not decided if we use the histogram yet. This paragraph needs to be rewritten once we are clear with the code.}}
...
The design of an neural network architecture highly depends on the problem at hand. In our case, we are trying to discriminate samples of two distributions cosmologicals maps. Hence, we assume that each pixel carries some information about the distribution. Thanks to 6 graph convolution layers (GC sizes: 16, 32, 64, 64, 64, 2) we first build the stistical evidence of each distribution for each pixel of a downsampled sphere. The last graph convolution layer is compose of 2 channels as we have 2 distributions. Then we use a statistical 'mean' layer (SL) to average the evidence for each distribution and we compute the probabilities thanks to a softmax layer (SM). We use batch normalization (BN), pooling (P) and relu non-linearity $\sigma$ for each convolutional layer. The network can be summarized as
\begin{equation}
NN = SM \circ ST_{\text{mean}} \circ L_6 \circ L_5 \circ L_4 \circ L_3 \circ L_2 \circ L_1
\end{equation}
where
\begin{itemize}
    \item layer 1: $L_1 = P  \circ \sigma \circ BN  \circ GC_{16}$,
    \item layer 2: $L_2 = P  \circ \sigma \circ BN  \circ GC_{32}$,
    \item layer 3-5: $L_3 = L_4 = L_5= P \circ  \sigma \circ BN  \circ GC_{64}$,
    \item layer 6: $L_6 = \sigma \circ BN  \circ GC_{2}$.
\end{itemize}

\todo{We use the ADAM optimizer, \cite{kingma2014adam}, with an initial learning rate of $2e-4$ that is exponentially decreased by $0.98$ a $100$ times to end at $2.7e-5$.
All models where trained $80$ epochs with a batch size of $16\times o^2$. With this choice, all models recieve an identical amount of data within a batch and were trained with $1920$ gradient steps.}
Training took less \nati{queue bottleneck to be fixed} than $3$ hours for each model using a single NVIDIA Tesla K20.
...
First, Chebyshev polynomial seems more stable than monomials ($\theta_k$ in \eqnref{graph_convolution_polynomial}), in the sense that the training loss had a more steadily decrease.
Nevertheless once the network is trained, we could not observe a significant accuracy difference.
Second, using a statistical layer slightly improved the accuracy in our problem.
...
\todo{Fourth, using $\ell_2$ regularization does not help neither with the performance nor the training of the models, but slightly reduce the performance variance between different runs due to the random initialization.
Eventually, recommend to ignitialize the Chebyshev coeffecient with a centered Gaussian distribution with standart deviation $2/(F * (K + 0.5))$, where $K$ is the Chebyshef order and $F$ the number of input channels. The
}
...
We remind the reader that the amplitude of the Laplacian eigenvalue is proportional to the sum of the variations of its associated eigenvector. Furthermore, all spherical harmonics with the same order $\ell$ have the same variation. Hence, the fact that the Laplacian eigenvalues are grouped by $2\ell + 1$ is an hint that the graph eigenvectors approximate the spherical harmonics.
In order to push the comparison one step further, we shows the correspondence (scalar product) between the subspaces spanned by the graph Fourier modes and the spherical harmonics in \figref{subspace_harmonics_eigenvectors}. 
...
Nevertheless, our experiments suggest that the theses downsides do not have an important effect on the convolution nor hinder the performance of the neural network.
...
In \figref{gaussian_filters_visualization}, we further leverage this example to present the three visualization of a HEALPix graph convolution kernel.
...
As a second visualization, we observe the kernel directly on the sphere. As the kernel $K_t$ is defined in the graph spectral domain, we convolve a Kroneker to obtain a representation on the sphere. We then plot the result with gnomonic projection. 
...
access to the same amount of data than the network, i.e, potentially an
