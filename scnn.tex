...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
	...
	\item Check names: we use HealpixNet, HealpixNet (CNN variant), HealpixNet (FCN variant)
	...
	\item (NP) Change equation after (2) and check all kroneker
	\item Notation: write element $i$ of vector $y$ as $y_i$, $y(i)$, or $y[i]$? Be consistent. Same for matrices. Keep indices.
...
A second direction attempted by~\citet{cohen2018sphericalcnn} was to leverage the spherical Fourier transform and to perform the SO(3) convolution in the spectral domain, thanks to the convolution theorem.
...
This is an important property as translation invariance is probably one of the main reason why classic CNN were successful in the first place.
...
While their architecture accommodates these invariances on the sphere and remains fairly efficient, they did not take advantage of a hierarchical pixelisation and use a grid discretisation of the sphere. 
...
\citet{cohen2018sphericalcnn} and \citet{esteves2017sphericalcnn} addressed the problem of rotation equivariance leveraging the convolution associated to the the 3D rotation group SO(3), with applications to atomization energy regression and 3D model classification, alignment and retrieval.
...
\citet{kondor2018clebsch} proposed to use the Clebsh-Gordan transform to perform a non-linear transformation in the spectral domain. As such, a SHT only has to be performed once at the input of the network and the back and forth SHT between every layer are no longer required. While this clever trick makes the convolution complexity $\bO(N_{pix})$, the non-linearity scales, to the best of our understanding, as  $\bO(N_{pix}^{3/2})$. At the time of writing this paper, computation time is unfortunately not reported, hence making unclear which speed gain is obtained in practice. 
While the convolution in the spectral domain avoid all sampling problems, one of its downside might the lack of natural locality, a property that is naturally obtained by the polynomial approach of graph convolution. Because the convolution is at the begging of the neural network, we believe that is should focus on local correlations first before exacting more complex structures.
...
\begin{figure}
	...
	\caption{Convolution kernel (also called filter) $h$ localized in the center and left corner of a surface. A filter is localized on pixel $i$ as $\T_i h = h(\L) \b \delta_i$ (see \secref{graph_convolution} and \eqnref{graph_convolution_spatial}). The graph is built from $1/12^\text{th}$ of the sphere at $N_{side} = 16$. The filter is not isotropic anymore when localized on the left corner due to border effects. The graph representation of a manifold assumes reflective border conditions.}
