...
\usepackage{enumitem} % to remove when done with todo list
 % to remove when done with todo list
...
The commonly used pixelzation scheme for spherical maps is Hierarchical Equal Area isoLatitude Pixelisation (HEALPix).
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
    ...
    \item (done!) (TK) check text figure 1
    \item (done!) (TK) Related work in cosmo. The only cosmological applications of spherical CNNs in the related work are about projection on the tangent plane \citep{schmelze2017cosmologicalmodel, fluri2018deep, gupta2018nongaussianinformation}. Aren't there applications in cosmology for the other (c.f. related work section) approaches? Most of them are illustrated in omnidirectional imagery. If none, say it and mention some alternatives, e.g., wavelets?
    \item (done!) (TK) check if papers which appear as arxiv were not published in a conference or journal
    \item (done!) (TK) read everything
    \item (fail - I checked few fonts here: \url{de.sharelatex.com/learn/Font_typefaces}, but none looked better) (TK) Font: do we have to use times (set in the documentclass)? I (MD) find the rendering of math a bit ugly.
    \item (done!) (TK) fix the bibliography
    ...
    \item (done!) (TK) Example applications for dense prediction (classification and regression) for cosmology (color text in 2.8)
...
A first direction to generalize CNNs to spherical data was to apply a standard 2D CNN to a grid discretisation of the sphere~\citep{boomsma2017spherical,su2017sphericalconv,coors2018spherenet}.
While this approach uses the well-developed 2D convolution and hierarchical pooling, it is restricted to specific grid sampling schemes that are highly irregular, and hence inherently deform the domain where the data is supported.
A second direction attempted by~\citep{cohen2018sphericalcnn} was to leverage the spherical Fourier transform and to perform the convolution associated to the SO(3) rotation group in the spectral domain, thanks to the convolution theorem.
...
Finally, the use of a graph to model the discretised sphere was considered by~\citep{khasanova2017graphomni}, towards the goal of designing a network that is invariant to rotations.
While its architecture accommodates these invariances on the sphere and remains fairly efficient, it did not take advantage of a hierarchical pixelisation and use a simple grid discretisation of the sphere.
...
This work is the first to design a spherical convolutional neural network for cosmological applications.
Previously, several other machine learning algorithms were extended to the sphere, often also including the third, radial direction.
Most notably, sparsity-based methods were developed, such as wavelets \citep{Leistedt2013flaglets,Leistedt2015spinwavelets}, ridgelets \citep{McEwen2015ridgelet} and curvelets \citep{Chan2017curvelets,Starck2009polarized}.
...
We introduce an efficient spherical CNN formulation which uses (i) convolutions on graphs and (ii) hierarchical pooling, while working with the Hierarchical Equal Area isoLatitude Pixelisation (HEALPix) sampling.
...
Our approach uses the graph CNN formulation introduced by \citep{defferrard2016convolutional}, which can accommodate any sampling of the sphere.
...
We give a practical demonstration of the application of our package to cosmological model discrimination using maps of projected mass distribution on the sky \citep{chang2017curvedsky}.
...
These models were designed to have very similar angular power spectrum for the angular wavenumbers $\ell < 1000$.
...
% Results show that our model is better at discriminating the maps and is more resilient to noise.
...
Following \citep{defferrard2016convolutional}, we define our filters as Chebyshev polynomials such as the convolution operation becomes
...
% The advantage of schemes based on a base polyhedron is that cells are equal-area. While that is not required for the convolution operation when modeling with graphs (it can be corrected by setting edge weights appropriately, see \citep{khasanova2017graphomni})
...
Following \citep{khasanova2017tigranet}, we implement an optional statistical layer between the last graph convolutional layer and the first fully connected layer for the neural network.
...
\begin{equation} \label{eqn:stat_layer}
	\y = f(\x) \in \R^{N_{stat}},
\end{equation}
where $f$ computes $N_{stat}$ statistics which are independent from the size of the map, $N_{pix}$. An example is the global average pooling $f(\x) = \esp(\x)$, introduced by \citep{lin2013globalavgpooling}, where $\esp(\x) = \frac{1}{N_{pix}} \sum_{i=1}^{N_{pix}} x_i$ is the empirical mean. Another example is $f(\x) = [\esp(\x), \var(\x)]\trans$, where $\var(\x) = \frac{1}{N_{pix}} \sum_{i=1}^{N_{pix}} (x_i - \esp(\x))^2$ is the empirical variance.
...
\end{equation*}
Such networks can be useful for
(i) dense regression, such as denoising, interpolating missing values, or inpainting a parts of a map \citep{Inoue2008inpainting}, or
(ii) dense classification, such as segmentation or feature detection \citep{Amsel2007detecting}.
We emphasize that the use of FC and ST layers is the sole difference between a neural network engineered for global or dense prediction.
...
\begin{figure}
    ...
    \caption[]{Some pixelizations of the sphere. Left: the equirectangular grid, using equiangular spacing in a standard spherical-polar coordinate system. Middle: an equiangular cubed-sphere grid, as described in \citep{ronchi1996cubed}. Right: graph built from an HEALPix pixelization of half the sphere ($N_{side} = 4$). By construction, each node has 8 neighbors, except the highlighted ones which have only 7.\footnotemark[7] Left and middle figures are taken from \citep{boomsma2017spherical}.}
...
The problem of rotation equivariance was addressed \citep{cohen2018sphericalcnn,esteves2017sphericalcnn} by leveraging the convolution associated to the 3D rotation group SO(3), with applications to atomization energy regression and 3D model classification, alignment and retrieval.
...
The Clebsh-Gordan transform was proposed \citep{kondor2018clebsch} to perform a non-linear transformation in the spectral domain. As such, a SHT only has to be performed once at the input of the network and the back and forth SHT between every layer is no longer required. While this clever trick makes the convolution complexity $\bO(N_{pix})$, the non-linearity scales, to the best of our understanding, as  $\bO(N_{pix}^{3/2})$. At the time of writing this paper, computation time is unfortunately not reported, hence making unclear which speed gain is obtained in practice.
While the convolution in the spectral domain avoid all sampling problems, one of its downsides might the lack of natural locality, a property that is naturally obtained by the polynomial approach of graph convolution. Because the convolution is at the begging of the neural network, we believe that it should focus on local correlations first before exacting more complex structures.
%\todo{Polynomial formulation enforces locality, which otherwise has to be enforced by smoothness in the spectral domain.}
%\todo{Pooling in the spectral domain? \citep{esteves2017sphericalcnn} do something.}
...
The use of a graph to model the discretised sphere was also considered by \citep{khasanova2017graphomni} for omnidirectional imaging.
...
% First, similarly to~\citep{boomsma2017spherical}, they use an equirectangular grid sampling.
...
The connection has been made between graph CNNs and group equivariant CNNs by \citep{kondor2018equivariance}, justifying the use of graphs to gain rotation equivariance on the sphere.
\todo{More here.}

Many formulations of graph neural networks, reviewed by \citep{bronstein2017review} and \citep{hamilton2017review}, have been proposed. For this contribution, we chose the formulation of \citep{defferrard2016convolutional} as its root on strong graph signal processing theory makes the concept of convolutions and filters explicit \citep{shuman2013emerging}. As the convolution is motivated by a multiplication in the graph Fourier spectrum, it is close in spirit, and empirically, to the formulation based on the spherical harmonics, which is the ideal rotation equivariant formulation.
...
The classification methods were trained to predict a label from an HEALPix map.
...
While we only demonstrate a classification task here, other tasks, such as regression or segmentation, are also possible (c.f. \secref{architecture}).
...
\begin{figure*}
	...
	\includegraphics[trim=1cm 0cm 0cm 0cm, width=0.49\linewidth]{figure_kappa_diff_model1.pdf}
	\hfill
	% \includegraphics[width=0.75\linewidth]{smooth_map_class_2}
	\includegraphics[trim=0cm 0cm 1cm 0cm, width=0.49\linewidth]{figure_kappa_diff_model2.pdf}
...
We make full sky N-body simulations for two parameter sets of the $\Lambda\rm{CDM}$ cosmological model: model 1 ($\Omega_m=0.31, \sigma_8=0.82$) and model 2 ($\Omega_m=0.26, \sigma_8=0.91$), where $\Omega_m$ is the matter density in the universe and $\sigma_8$ is the normalisation of the matter power spectrum.
...
We also subtract the mean of each map and down-sample them to a resolution of $N_{side}=1024$.
...
\figref{map_sample} shows the full sky simulations and a zoom region for both models.
...
\todo{thanks to stationarity, quantity of information is constant. Motivation: partial sky maps. Larger surfaces improve confidence in the prediction, the average trick.}
All pixels are as informative as any other.
Stationarity of N-body simulations.
\TK{Not sure what is meant here}

% While the number of maps might sound small for deep learning, the information contained in the maps is very redundant, allowing us to use parts of the map as samples for our simulation.
As many cosmological applications will use datasets located on part-sphere only, we use parts of the map as independent samples for our analysis.
Using the properties of the HEALPix sampling, we can split each map into $12 \times o^2$ ($o=1,2,4,\dots$) samples that span a smaller part of the sphere.
...
The standard deviation of the noise varies from $0\times$ (i.e., no noise) to $2\times$ the standard deviation of the noiseless maps.
...
The two baselines are based on features that are (i) the power spectral densities (PSD), and (ii) the histogram of pixels in the maps \citep{patton2017cosmologicalconstraints}.
...
We use the Adam optimizer \citep{kingma2014adam} with $\beta_1=0.9$, $\beta_2=0.999$, and an initial learning rate of $2 \times 10^{-4}$ that is exponentially decreased by a multiplication with $0.999$ at each step.
...
% The CPU is probably slow which explain the 5 hours. I believe we could be much faster than that by computing less summaries and improving the data. But, Michael, it's OKAY to have a margin of improvement. :-)
...
\begin{figure*}
	...
	\caption{
		Classification errors that compare the performance of the fully convolutional (FCN) and the standard convolutional (CNN) variants of the HealpixNet architecture, the support vector machine (SVM) with the power spectral densities (PSDs) as features, and the SVM with histogram as features.
		The difficulty of the task depends on the level of noise and the size of a sample (that is, the number of pixels that constitute the sample to classify). Order 1 corresponds to samples which area is $1/12=8.1\%$ of the sphere, order 2 to $1/(12 \times 2^2) = 2.1\%$, and order 4 to $1/(12 \times 4^2) = 0.5\%$. The standard deviation of the added Gaussian noise varies from zero to $2\times$ the standard deviation of the noiseless maps.
		It is clear from those results that the noise makes the problem harder, and that having more pixels available to classify a sample makes the problem easier (the classifier having more evidence to make a decision). The FCN variant of HealpixNet beats the CNN variant by exploiting rotation invariance. Both variants largely beat the two SVM baselines.
	}
	\label{fig:results}
\end{figure*}
...
We observe that the PSD is unable to perform the desired task, with accuracy decreasing significantly for all noisy cases.
...
The performance of the histogram features is very good for larger maps and deteriorates for the smaller cases with increased noise level, reaching 80\% at the highest noise level for $o=4$.
...
We also observe that the FCN architecture slightly outperforms the CNN one.
This may seem counter-intuitive as the CNN is a generalization of the FCN and hence should provide, at least, equivalently good results.
Nevertheless, one should not forget that in practice the complexity of the architecture trades with more data requirements and more difficulties in the training.
We have two interpretations that explains why FCN architecture leads to better performance.
First, the network is almost invariant to rotation on the SO(3) group and hence intrinsically augments the dataset.
The convergence mass map dataset is likely to be rotational invariant as the mass distribution is homogeneous and isotropic.
Second, the FCN architecture can be interpreted as a voting procedure where
(i) each pixel is being classified using the data in its surroundings, and
(ii) the average score is computed to determine the class.
This procedure suits the data as we believe that each class of the dataset is stationary with a small-support radial autocorrelation function, again stemming from the homogeneity of the dataset.
It implies that a pixel can be statistically classified using only its surroundings.
For other datasets not having these two properties the CNN architecture may be better and one should always, when possible, adapt the architecture to the data.
...
\TK{I have a similar impression!}
...
% Notably, HealpixNet maintains good classification accuracy even for high noise levels and part-sky areas.
...
We would like to thank Alexandre Refregier, Adam Amara, Thomas Hofmann and Fernando Perez-Cruz for advice and helpful discussions.
...
The original number of samples $480$, $1920$,
...
% The validation set remain the same and is used for cross-validation to tune the L2 penalization parameter of SVM.
% Eventually, we evaluate the final classifier on the test set.
