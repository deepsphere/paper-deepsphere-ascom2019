...
% MD: useful info? To me this is not an argument for our approach. Maybe we have to expand it?

% Architectures are then designed similarly to the classical CNNs used for image analysis.
% HealpixNet uses Tensorflow as the underlying engine and can utilize most of it functionalities.
% MD: not relevant for the abstract in my opinion
...
Cosmological data often comes as spherical maps, which make the use of traditional CNNs more complicated.
...
With filters restricted to being radial, our convolutions are equivariant to rotation on the sphere, and HealpixNet can be made invariant or equivariant to rotation.
...
% An advantage of the graph representation is that the convolution operations are efficient, which enables the CNN to run fast.
...
Our experimental results show that the performance of HealpixNet is always superior or equal to both of these baselines.
...
Code, data, and examples are available at \url{https://github.com/SwissDataScienceCenter/HealpixNet}.
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
	...
	\nati{No answer from the people. Let us go for DeepSphere. Graphsphere???}
	\item (TK) Check use of British vs American English. Check use of tense.
	\item (MD) Figure 4: speed for multiple lmax. Not doing it.
    \item (TK) check part sphere notebook and whole sphere
	\item (NP \& MD) Move the git repository, rename the classes, check the links.
...
% MD: I don't see what the following brings to the paper.
...
% Compared with~\citep{khasanova2017graphomni}, our work does not need hand-chosen statistics nor dynamic pooling.
...
Code and examples are available online.{\interfootnotelinepenalty=10000 \footnote{\url{https://github.com/SwissDataScienceCenter/HealpixNet}}}

% MD: I usually find those paragraphs boring and skip them when reading. Our intro is long enough already. Feel free to add it back if you think it's necessary.
%The paper is organized as follows.
%In \secref{related_work}, we describe existing approaches to convolutions on spheres, graphs, and manifolds.
...
\end{equation}
% NP: Should we take a single vertex and explain what happens to it?
...
% MD: SGD implicitly imposes a uniform prior on the learned parameters.
% Moreover, they factor out $\tL^{k-2}$ from $T_k(\tL)$, e.g., the 3-neighborhood is not mixed with the 5-neighborhood.
% That results in an easier optimization of the parameters $\theta$.
...
% \todo{numerical stability of $\L^k \f$ v.s. $T_k(\L) \f$}
% NP: we should not give much more details here. If we do an experiment, we can comment in the experiment section.}
...
% In our case, the Laplacian is extremely sparse, with 8 to 9 entries per line (out of $N_{pix}$).
...
% MD: Nope, the graph can adapt to any sampling. And we're not arguing about sampling schemes.
...
% Both our Python implementation{\interfootnotelinepenalty=10000 \footnote{\url{https://github.com/SwissDataScienceCenter/HealpixNet}}} and the data\footnote{\url{https://doi.org/10.5281/zenodo.1303272}} to reproduce those experiments are openly available online. Furthermore, Jupyter notebooks demoing the method and problem can be run from your browser without installing anything.
Our Python implementation{\interfootnotelinepenalty=10000 \footnote{\url{https://github.com/SwissDataScienceCenter/HealpixNet}}} to reproduce those experiments are openly available online.
The data is available upon request from the cosmology group\footnote{\url{www.cosmology.ethz.ch}}.
Furthermore, Jupyter notebooks demos can be run from your browser without installing anything.
...
The simulations are created using the fast lightcone method \pkg{UFalcon} described in \citep{sgier2018fastgeneration}. A brief overview about the map making procedure used in \pkg{UFalcon} is given in \ref{sec:convergence_mass_maps} and the specific simulation setup is given in \ref{sec:simulation_setup}.
...
% dense: segmentation tasks, such as masking, flagging, or object detection
...
% Fields: planets, molecules?, atoms.
...
In the following we describe the weak lensing map-making pipeline \pkg{UFalcon} (ultra fast lightcone) developed in \citep{sgier2018fastgeneration} and how it has been used to generate data used in the present work. The data consist of two-dimensional convergence maps on the full sphere, which represent the dimensionless, weighted and projected mass along the light of sight and determines the lensing of source galaxies an observer located on earth would see.

The matter content of the universe consists mostly of dark matter and is therefore mainly responsible for the large scale structure evolution of the universe. The effect of weak gravitational lensing, in which the light from distant galaxies is lensed by the matter distribution between the observer and the sources, is therefore dominated by dark matter \citep[see][for a review of gravitational lensing]{bartelman2010gravitationallensing}. In order to obtain a convergence map characterising the gravitational lensing of distance sources, one has to start with accurate simulations of the dark matter distribution. This is achieved through cosmological N-Body simulations, which evolve a chosen number of dark matter particles $N_\mathrm{part}^\mathrm{sim}$ within a specific simulation volume $V_\mathrm{sim}$ under the influence of gravity across cosmic time-scales. The simulation outputs the positions of the particles at user-specific time-steps. These time-steps correspond to redshifts $z$, by which the spectrum is shifted the observer would measure. A redshift of $z=0$ correspond to the present time and higher redshifts to earlier times.

Since the convergence map characterises the dark matter lenses at different redshifts, one has to first construct a lightcone. The lightcone represents the volume of the universe around the observer, where the radius represents the distance the light from distant galaxies at different times and locations has to travel to the observer. Therefore, the particles outputted at different redshifts (time-steps) of the simulation are concentrically arranged in shells of constant redshift around the observer (each redshift $z$ corresponds to a comoving distance away from the observer $\chi(z)$), which is located at redshift $z=0$ (see Figure \ref{sketch_lightcone_simulation}). The shells are then weighted by a weak lensing specific weight $W_b$ and projected onto the sphere for each pixel $\theta_\mathrm{pix}$, resulting in a convergence map
\begin{equation}
