...
proposed by~\cite{cohen2018spherical}. The goal of
...
set (theoretically including HEALPix, \cite{mohlenkamp1999fast,rokhlin2006fast}). \todo{HEALPix was designed to have a fast SHT, by being iso-latitude.}
...
acceleration, the convolutions remain expensive, limiting the practical use of this approach.
...
\todo{@nath: I found (i) $\bO(\ell_{max}^3)$ in \url{https://arxiv.org/pdf/1202.6522.pdf} and (ii) $O(N^2 \log(N) \log(1/\epsilon))$ for $N^2$ pixels and precision $\epsilon$ in \url{http://tygert.com/sph2.pdf}. From what I understood (ii) is asymptotically fast, but has large constants and as such (i) always wins in practice. Not sure what we should use here. And I don't know either what healpy is using. Maybe we can leave the complexity analysis for a more theoretical paper.}
\figref{filtering_speed} shows that a naive implementation of our method is more than ten times faster than the wildly used \pkg{healpy} implementation which uses a fast spherical transform for which the HEALPix sampling was designed for.
