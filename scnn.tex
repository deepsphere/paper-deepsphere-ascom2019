...
\begin{itemize}
    ...
    \item Update result section according to the new simulations...
...
set (theoretically including HealPix, \cite{...}). The main advantage of this
...
We compare the performance of the spherical CNN to two simple benchmark algorithms.
The two baselines are based on building features with a) the Power Spectral Densities or b) the histogram of pixels in the maps.
Those features are then classified use a Support Vector Machine (SVM) with a linear kernel. Other kernels did provide better result while having worse scaling properties.
For fairness, we augmented the dataset of the baseline in a similar way as for the network, i.e: 
...
Both the PSD features and the CNN can exploit the spatial information.

For The maps
The performance is assessed by measuring the classification error on a test set.
