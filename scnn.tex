...
\nati{I am not that we should talk about pooling in the abstract. Another angle would be to say that HealPix sampling is well suited for spherical CNN with graphs as a) it is very regular => the graph capture well the spherical Manifold structure and b) pooling is natural.} 
While convolution on arbitrary graphs has mostly been sorted out, a general pooling operation has not been discovered so far. 
...
\nati{@mdeff: for the complexity we probably have $\mathcal{O}(n)$ instead of $\mathcal{O}(n^{3/2})$ (let assume that we have $\approx n^{1/2}$ points allong both axis, computation of the SO3 FFT is composed of 1) $n^{1/2}$ FFTs for a total cost of $n \log (n)$ and 2) $n^{1/2}$ matrix multiplications of size $n^{1/2}$ for a total cost of $n^{3/2}$)}
}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.45\textwidth]{figures/hist_error_order2_noise1.pdf}
\caption{Evolution curve of the error with respect of the number of training sample for histogram features and a linear classifier (setting: order 2, relative noise level: 1). The validation error is slightly below the training error because we use the validation set for the cross-validation and hence we are probably slightly overfitting the validation set.}
