...
An advantage of this representation is that the convolution operations with this approach are efficient, which enables the algorithm to run fast.
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
	...
	\item (MD) remove statistical layer
	\item (MD) distinction between CNN and FCN => Variants subsection
	...
	\item (done!) (MD) check paragraph on our chosen architecture in the Experiments sections
\end{itemize}
...
This is possible as CNNs have the capacity to build rich models and capture complicated, non-linear patterns, which are often present in the data.
...
As such, we can control the difficulty of the classification problem by limiting the number of pixels available to the algorithms, i.e., extracting partial maps from full sky maps.
...
%\subsection{Methods} % baselines and HealpixNet together
\label{sec:selected_architecture}

As discussed in \secref{variants}, the choice of an HealpixNet variant depends on the data and task at hand.
...
The chosen network is defined as
\begin{equation*}
	FCN = SM \circ ST_{\text{mean}} \circ GC_2 \circ L_5 \circ L_4 \circ L_3 \circ L_2 \circ L_1,
\end{equation*}
\begin{center}
...
\end{tabular}
\end{center}
where $GC_k$ indicates a graph convolutional layer with $k$ output feature maps, $P_4$ is a pooling layer which divides the number of pixels by 4, $\sigma$ is a ReLU, and the Chebyshev polynomials are of degree $K=5$.

For comparison, we tried the more conventional CNN variant, where the classification is performed by a fully connected layer from the last feature maps instead of the average of local classifications. This architecture is not invariant to rotation. The network is defined as
\begin{equation*}
	CNN = SM \circ FC_2 \circ L_5 \circ L_4 \circ L_3 \circ L_2 \circ L_1,
\end{equation*}
where $FC_2$ denotes a fully connected layer with two output neurons.
Architectures were chosen over their performance on the validation set.
...
% Could probably be faster with a better input pipeline.
...
We also observe that the FCN variant slightly outperforms the CNN. This may seem counterintuitive as the CNN is a generalization of the FCN and hence should provide, at least, equivalently good results.
Nevertheless, one should not forget that in practice the complexity of the architecture trades with more data requirements and more training difficulties.
Two interpretations explain why the FCN variant leads to better performance. First, the FCN is almost invariant to rotation on the SO(3) group and hence intrinsically augments the dataset.
The convergence mass map dataset is invariant to rotation as the mass distribution is homogeneous and isotropic.
Second, the FCN can be interpreted as a voting procedure where (i) each pixel is being classified using the data in its surroundings, and (ii) the average score is computed to determine the class.
...
The CNN variant may however be better for data not having this property, and one should always adapt the architecture to the data and task.

While testing the FCN and CNN variants, we made the following empirical observations.
First, training was more stable (in the sense that the loss had a more monotone decrease) when using Chebyshev polynomials, as in \eqnref{graph_convolution_cheby}, rather than monomials, as in \eqnref{graph_convolution_monomial}.
...
\begin{figure}
	...
	\caption{Random selection of 24 learned filters from the fifth spherical convolutional layer $L_5$. Top: section. Bottom: gnomonic view.}
...
While those particular filters were obtained from the experiment with order $o=2$ and a relative noise level of $2$, all trained networks presented visually similar gnomonic and profile patterns.
% The gnomonic projection clearly exhibits the sampling issue of HEALPix.
...
% Nati: Note that filter are almost not evolving during the training process. It is not surprising as our architecture is likely to be overparametrized. If we want to have interpretable filters, we probably have to use a much smaller architecture.
