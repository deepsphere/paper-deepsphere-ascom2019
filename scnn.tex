...
The softmax layer is the last layer in a neural network engineered for classification.
...
The goal of training is to find the parameters $\theta$ of the neural network that minimize the risk $R(\theta) = \esp \left[ C \left( NN_\theta(\X), \bar \Y \right) \right]$, where $\esp$ is the expectation over the joint distribution $(\X, \bar \Y)$.
...
\nati{
Additionally, we compare DeepSphere to a traditional 2-dimensional convolutional neural network (2D ConvNet).
To do so, the samples are first transformed into a square image before being fed into the 2D ConvNet.
...
We used convolution kernel of size $5\times 5$ and reduced the number of filters in order to match the number of variables between DeepSphere and the 2D ConvNet.
}
...
\begin{figure*}
	...
	\caption{
		Classification accuracy of the fully convolutional (DeepSphere FCN) variant of DeepSphere, the standard convolutional (DeepSphere CNN) variant of DeepSphere, the fully convolutional ConvNet (2D ConvNet FCN), the standard convolutional ConvNet (2D ConvNet CNN), the support vector machine (SVM) with the power spectral density (PSD) as features, and the SVM with the histogram as features.
...
\nati{
To our surprise, the traditional 2D ConvNet did not match the performance of DeepSphere.
It can probably be explained by the fact that the 2D ConvNet breaks the rotational invariance of the data.
...
This standard deviation has the property of keeping the energy of the signal more or less constant across layers.\footnote{We derived this rule from the Chebyshev polynomial equations, the principles presented in \cite{glorot2010understanding}, and some empirical experiments.}
...
As a second visualization (middle), we observe the kernel directly on the sphere. As the kernel $K_t$ is defined in the graph spectral domain, we convolve a Kronecker $\delta$ to obtain a representation on the sphere. We then plot the result with a gnomonic projection.
\nati{For our third visualization (bottom), our goal is to plot the section of a filter. This visualization might be convenient as we theoretically have spherical filters (up to the irregularities of the sampling). The idea is to plot the section of the convolved Kronecker $\delta$.}
To do so, we convolve a Kronecker $\delta$ on the vertices along the equator and use the equator for our section.
Note that, because of the small irregularities in the HEALPix sampling, the second and the third methods are subject to small variations depending on the chosen position of the Kronecker $\delta$.
...
In order to compare the DeepSphere architecture with a 2D ConvNet, we need to project the spherical data onto a plane.
