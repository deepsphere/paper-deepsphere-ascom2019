...
% Hence, we present a spherical CNN for analysis of full and partial HEALPix maps, which we call DeepSphere.
...
% As such, while DeepSphere is a special case of a graph CNN, it is tailored to the HEALPix sampling of the sphere.
...
% The filters learned by the DeepSphere are restricted to being radial.
...
% DeepSphere achieves \todo{xx\%} better accuracy than those baselines, is more robust to noise, and is \todo{faster?}.
...
As the Equal Area isoLatitude Pixelisation (HEALPix)~\citep{gorski2005healpix} is a popular sampling used in cosmology and astrophysics, we tailored the method to that particular sampling and call it DeepSphere.
...
% \footnote{The variance of the averaged prediction is $\sigma^2 / n$, where $\sigma$ is the variance over one observation and $n$ is the number of independent observations.}
% TK: no need for this footnote, it should be general knwoledged for the readers of this journal
...
%\subsection{Methods} % baselines and DeepSphere together
...
% and one should always adapt the architecture to the data and task (see \secref{architecture}).

% We saw no sign of over-fitting in any of our experiments.
% With more data, DeepSphere can probably do even better.
...
% TK: removing this paragraph, sounds too spectulative <- not strictly necessary for the paper
...
especially when one desire to build a classifier that is robust to noise.
As a result, we use a classical dataset augmentation technique.
...
For a fair comparison between DeepSphere and the baselines, we have to ensure that the SVM classifier has access to the same amount of training data as DeepSphere. That is potentially an infinite number of samples. Hence, we fit different linear SVM classifiers using various training set sizes until we experimentally observe that increasing the amount of training data does not improve performance. An example convergence of the training and validation errors is shown in \figref{hist_error_evolution}.
