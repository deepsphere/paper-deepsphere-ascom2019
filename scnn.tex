...
\citet{cohen2018sphericalcnn} and \citet{esteves2017sphericalcnn} addressed the problem of rotation equivariance by defining the convolution on the 3D rotation group SO(3), with applications to atomization energy regression and 3D model classification, alignment and retrieval.
...
\citet{kondor2018clebsch} proposed to use the Clebsh-Gordan transform to perform a non-linear transformation in the spectral domain. As such, a SHT only has to be performed once at the input of the network and the back and forth SHT between every layer are no longer required. While this clever trick make the convolution complexity $\bO(N_{pix})$, the non-linearity\nati{ scales as the squared of the number of channels and as }. As a result, it is yet unclear which speed gain is obtained in practice. \todo{computation complexity? no speed comparison in paper?}
