...
While CNN apply to images, in many applications, the data comes in a form of spherical maps with the popular Healpix format, which uses equal-area, isolatitude sampling.
Hence, we present a spherical CNNs for analysis of full- and partial sky Healpix maps, which we call HealpixCNN (???) \nati{HealpixNet??? To be decided}.
Our approach is based on representing a sphere as a graph of connected nodes.
...
Spectral Densities or b) the histogram of pixels in the maps. CITE~\cite{...}
After a normalization, those features are then classified use a Support Vector
Machine (SVM) with a linear kernel. Other kernels did provide better result
...
error converge to a values. This process is depicted in
Figure~\ref{fig:hist_error_evolution}.
...
technique\footnote{We also give a small advantage to the SVM classifiers by
...
The histogram features contains information about the distribution of pixels, but does not include any spatial information.
In contrast, both the PSD features and the CNN can exploit the spatial information.
