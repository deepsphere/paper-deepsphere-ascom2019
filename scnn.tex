...
The experiment presented here is similar to the one by \citet{schmelze2017cosmologicalmodel}.
...
The regression task will most likely be the most practical cosmological application, as described in \citet{gupta2018nongaussianinformation} and \citet{fluri2018deep}.
...
The simulations are created using the fast lightcone method described in \citet{sgier2018fastgeneration}.
...
\begin{figure*}
	...
	\caption{
		...
		The difficulty of the task depends on the level of noise and the size of a sample (that is, the number of pixels that constitute the sample to classify). Order 1 corresponds to samples which area is $\frac{1}{12}=8.1\%$ of the sphere, order 2 to $\frac{1}{12 \times 2^2} = 2.1\%$, and order 4 to $\frac{1}{12 \times 4^2} = 0.5\%$. The standard deviation of the added Gaussian noise varies from zero to $2\times$ the standard deviation of the noiseless maps.
...
We also observe that the FCN architecture slightly outperforms the CNN one. This may seem counterintuitive as the CNN is a generalization of the FCN and hence should provide, at least, equivalently good results. Nevertheless, one should not forget that in practice the complexity of the architecture trades with more data requirements and more difficulties in the training.
We have two interpretations that explains why FCN architecture leads to better performance for two reasons. First, the network is almost invariant to rotation on the SO(3) group and hence intrinsically augments the dataset. The convergence mass map dataset is likely to be rotational invariant as ... \nati{Tomek: could you justify this from a cosmological point of view?}.
Second, the FCN architecture can be interpreted as a voting procedure where 1) each pixel is being classified using the data in its surroundings, and 2) the average score is computed to determine the class.
This procedure suits the data as we believe that each class of the dataset is stationary with a small-support radial autocorrelation function. \nati{Tomek: can you justify this from a cosmological point of view?} It implies that a pixel can be statistically classified using only its surroundings.
