...
To be able to train the network efficiently, the convolution must also be efficient and fast to perform.
...
A second attempt by~\citet{cohen2018spherical} was to leverage the spherical Fourier transform and to perform the convolution in the spectral domain, thanks to the convolution theorem. While there exists fast spherical Fourier transforms for some samplings, this approach can still be computationally expensive.
...
Other statistical layers are possible. In particular, we also implemented a learned histogram from \citep{wang2016learnhist}. Additionaly \citep{khasanova2017tigranet} propose to apply differen diffusion on the final maps to obtain statistics at different sclaes.
...
To remove this information, we additionally smooth the spherical maps with a Gaussian kernel of radius $3$ arcmins. The resulting PSD are displayed in \figref{psd_sigma3}. In the pre-processing step, we also remove the mean of each map and downsample them to a resolution of of $N_{side}=1024$.
...
After a normalization, those features are then classified using a Support Vector
Machine (SVM) with a linear kernel. Other kernels did not provide better results
while having significanly worse scaling properties. All classifiers are trained with noisy samples. For fairness, we augmented the dataset
...
error converge to a value. This process is detailed in \ref{sec:dataset_augmentation}. We tuned the regularization parameter by cross-validating with the validation set.
...
histogram features allow for a significant improvement over the PSD ones.
...
We have many different spherical neural network architectures and made a few empirical observations.
First Chebyshef polynomial seems more stable than monomials. Nevertheless once the network is trained, we could not observe a significant accuracy difference.
Second, using a statistical layer improved slightly the result for our specific problem. We tried $4$ different layers `mean', `variance', `mean + variance' and `histogram'. All but the `variance' were providing the same range of accurancy, the variance being slightly worse.
Third, dropout did not help in the precence of noise. This is probably du to the fact that the dataset was augmented indefinitely
Fourth, especially for difficult problem (order 2 or 4, and the presence of large noise) having a large architecture (many filters) and doing many iterations were necessary to achieve the result presented.
...
of HEALPix.
For a detail, on how the convolution kernels are plotted, please refer to \ref{sec:filter_visualization}.
