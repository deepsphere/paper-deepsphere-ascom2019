...
Neural networks are constructed as stacks of layers which sequentially transform the input data. Our architecture, pictured in \figref{architecture}, is composed of the following types of layers: graph convolutional (GC) layers, non-linearity, pooling (P), batch normalization (BN), statistical layer (ST), and fully connected (FC) layers.
...
The softmax layer is the last layer in a neural network engineered for classification. Given the output $\x \in \R^{N_{classes}}$ of the next to last layer, called the logits in the deep learning literature, the softmax layer outputs
\begin{equation*}
