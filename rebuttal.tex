...
\begin{itemize}
  ...
  a) I think that SVM fails (I think this this an overfitting problem. This is the case at least for the noiseless case, TO CHECK) I was able to run SVM, it works, but the result is pretty bad. TODO: do a full experiment.
  ...
  The input number of pixel for our order $1$ is $1024^2$ which severly limits the number of neurons that can be used in a fully connected architecture. For the first layer, we were only able to $128$ neurons before hitting an out of memory errors. In this setting, with an relative amount of noise of 0.5, we were unable to find regularization setting for the network such that it would actually classify the validation set. Note that all calssifiers performed well in this particular setting. Similar conclusion were observed with other setting, even when increasing the number of neurons for order $2$ and $4$.
...
\todo{@nati
We were actually limited in resolution when computing the graph Fourier eigenvectors as diagonalizing the Laplacian matrix scales as $O(N_{pix}^3)$. Hence we used a resolution of $N_{side}=16$. The information has been added to the paper. We believe that, with the correct weighting scheme the graph eigenvectors may converge toward the spherical harmonics as the resolution increases. We also believe that the eigenvector associated with the lowest $\ell$ "converges first". }
...
\begin{itemize}
  \item No here we only care about filter speed. 
  \item Filters do not have to be the same as they are learned anyway by the network.
  \item Replacing healpix filtering with graph filtering might be possible (suggested by Appendix C and fig 14), but is probably not usable when precision is required. It is left for futur work.
\end{itemize}
}
