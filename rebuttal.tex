...
\end{mdframed}
We are not sure what the reviewer means here. $\mathbb{R}_+$ is the ensemble of the real positive numbers and $h$ is simply a function that maps any positive real numbers to another real number.
...
\end{mdframed}
% Here the notion of neighboorhood is defined in the context of graphs. Hence there is no coordinates to compute distances.
...
\end{mdframed}
\todo{Assigned: @michael}
\nati{Here I do really understand this: the text reads:

\textit{For a classification task, the cost is usually taken to be the cross-entropy
\begin{equation*}
...
If you do a global prediction, you ar for sure in a regression task right? So I guess the two last sentence should be merged.
}
