...
Dear reviewer, we thank you for reading our paper and providing comments. They have helped us improving our manuscript. In this rebuttal, you can find a detailed answer to all the points you mentioned. 
...
The intuition of the reviewer is correct, theorectially using the raw data, \emph{an SVM or even a standard fully connected neural network (NN) would perform a lot better.} In practice, this those models are limited by the computational power and the dataset size. 

We had actually tried training an SVM on the raw data but were unable to obtain over $60\%$ accuracy in cases where all the other models were close to $100\%$. The classifier would either overfit or be over-regularized. For that reason, we decided, not to report the SVM results in the paper.  \nati{FYI, I did try again to be entirely sure about it.}

A model based on a fully connected neural network, as suggested by the reviewer, will contains a very large amount of weights severly limiting what can be tested with conventional hardware. In our case the input number of pixels (for order $1$) is $1024^2$ which severly limits the number of neurons that can be used in the architecture. For the completeness of this answer, we did try a fully connected NN. Practially, in the first layer, we were only able to $128$ neurons before hitting an out of memory errors. In this setting, with an relative amount of noise of $0.5$, we were unable to regularize the network such that it would actually classify the validation set. Note that all calssifiers performed well (close to $100\%$ accuracy) in this particular setting. Similar conclusion were observed with other setting, even when increasing the number of neurons for small part of the sphere (order $2$ and $4$).

This being said, the reviewer comment is justified and we will add these information to the paper. 

\todo{Add it in the paper... Make clear that CNN is so much better than fully connected + say that SVM on he raw features does not work.}
...
\end{mdframed}
We are aware that this particular point could be more detailled. Nevertheless, we believe that a deeper discussion does not belong to this paper. In fact, we are currently investigating the convergence question with the goal of obtaining better graph constructions or rotational equivariance results.
...
Since we do not have any convergence result yet, we are not usure about how the projection error evolves. However, if there is convergence (and we believe there is), then the eigenvector associated with the lowest $\ell$ probably converge first.
...
In the experiment shown if Figure 4, we only care about speed and not about the filter itself (we could have taken any filter). The main reason is that in the context of neural network, filters will be learned anyway. Libsharp is probably not optimized for Gaussian smoothing, but this experiment should not be limted to Gaussian filter. We intend to report on the general filter speed and use conveniently a Gaussian.
...
Our feeling is that in its current state, it probably does not have the required precision to be usable in a high precision setting. Nevertheless, theoretical improvement on the convergence side could definitly makes it competitive.
...
In term of neural network, the availlable implementation of \cite{cohen2018sphericalcnn,kondor2018clebsch} for spherical CNNs and Clebsh-Gordan CNNs are not scalable enough to be trained on the cosmological maps dataset and do not use the HEALPix sampling. Hence a comparison with thee methods requires a different setting, which far beyond the scope of our contribution. Nevertheless, this comparizon is important and we preparing a second contribution where it will be conducted.
...
\begin{itemize}
  \item We will do this comparizon...
...
Theoretically, providing that the FCN and the CNN have the same modelization power (i.e, the FCN does not have more or larger layers) and assuming the convolution would be Truly invariant to rotation, the CNN could never exceed the FCN for truly rotational invariant data. However, assuming enought augmentation (as suggested by the reviewer) and training, the CNN should get to the level of the FCN. In practice, this would require more training as the CNN has to learn the invariance. 
The point of defining a FCN instead of a CNN is to avoid the complexity of making this augmention. Hence we did not implement it. Nevertheless, it is an interesting point worth discussion in the paper. We updated the manuscript accordingly.
\todo{Do it}
...
\end{mdframed}
Thank you very much for your comments. We adressed them to the best we could.
...
As suggested in the previous, the weighting scheme used is very important for our laplacian to approach the true Laplace Beltrami operator. Using a more appropriate weighting scheme, one could hope for a more rotational invariant convolution and a faster convergence rate of the opertor. 
...
We agree with the reviewer. We actually misformulate the this sentence. We have reformulate it as:
\begin{mdframed}[style=manuscript] 
...
\end{mdframed}
In the work of \cite{kondor2018clebsch}, the convolution is perfomed in the spectral (harmonic) domain with a simple Adamar product. Instead of recomposing the signal and applying a traditional non-linearity such as "relu" in the pixel domain, \cite{kondor2018clebsch} propose to apply directly a Clebsch–Gordan product in the spectral domain. As a non-linear operation preserving the rotation equivariance, it is considered as the non-linearity of the neural network.
...
\end{mdframed}
\nati{Should we be more clear in the paper?}
...
\end{mdframed}
When comparing the speed, we used a single core CPU implemention of the graph convolution. We believe that this comparizon is fair.
\nati{Is this sufficent?}
...
\end{mdframed}
We are thinking about it. However, as the graph framework we are using only works with radial filters, it is not a simple question. One simple trick to make network sensitive to direction is to add a per channel fully connected layers. We are currently investigating other potential graph techniques that would lead to non-radial kernels.
...
\end{mdframed}
\todo{Assigned @tomek. 
This reviewer really spend the time. We should say something.}
...
\nati{@all, when you read the paper, please check that I did not forget some of them. Every layer has be changed to math symbole as we use them like function. For example SM becomes $SM$.}
...
\begin{mdframed}[style=manuscript] 
We consider the sky maps rotationial invariant as in our case rotating maps on the sphere doesn't change their interpretation since only the statistics of the maps are relevant.
\end{mdframed}
...
\end{mdframed}
In fact, $p$ is defined as $p=\log_2(N_{side} / N'_{side})$. We have updated the manuscript with this clarification and with an equation environnment.
...
\nati{The reviewer is correct. Our justification is shaddy. I am not sure how to make this clear, without writting too much. It is kind of a general consent that smoothness in one domain implies loclization in the other. The reason is that for the continuous Fourier transform, we have:
\begin{equation*}
...
\end{equation*}
Hence the theorem says that you cannot concentrate abritrarily in one domain without de-concentrating in the other. 
...
\end{mdframed}
\todo{Assign: @nati, @michael}
\nati{We would need an appendix to do that. It is the last figure of the figures notebook (expected filter). I could spend two hours and write some equation... @michael, what do you think?}
...
\end{mdframed}
\todo{Assign: @all}
\nati{The problem is that random filters are picky as well and look very similar. Should we say that? I to not think a colorbar is necessary since there is number on the top plot. Do you agree?}

We decided to not add a colorbar as scale information is already present in the top plot of the figure. 
...
\end{mdframed}
We have corrected this mistake according the reviewer suggestion.
...
\begin{mdframed}[style=manuscript] 
Furthermore, the harmonics ressemblance is another sign that the constructed graph is able to capture the spherical
...
\end{mdframed}
We have merged the three plot a an signal figure and adapted the text reference accordingly.
