...
% ... is a measure of the variation of the eigenvector $\boldsymbol{u}_i$ is on the graph defined by the Laplacian L.

%  - I don't think the "is" after $\boldsymbol{u}_i$ should be there.
...
% Similarly, each line of the matrix $\sum_{k=0}^&#x10fc04;K\theta_k\boldsymbol{L}^k$ defines an irregular patch of radius $K$.

%  - What do you mean by "line"? Is it a row or a column or something else which I've misunderstood?


% ... where $\tilde{\boldsymbol{L}}= \frac{2}{\lambda_{max}}\boldsymbol{L}−\boldsymbol{I}=−\frac{2}{\lambda_{max}}\boldsymbol{D}^{−1/2}\boldsymbol{W}\boldsymbol{D}^{−1/2}$ is the rescaled Laplacian with eigenvalues $\tilde{\Lambda}$ in [−1, 1].
...
% Given a map $\boldsymbol{x}\in\mathbb{R}^{N_{pix}}$, pooling defines $\boldsymbol{y}\in\mathbb{R}^{N'_{pix}}$ such as

%  - It should be "such that" not "such as". "Given a map $\boldsymbol{x}\in\mathbb{R}^{N_{pix}}$, pooling defines $\boldsymbol{y}\in\mathbb{R}^{N'_{pix}}$ such that"
...
% Note that the output $\boldsymbol{Y}\in\mathbb{R}^{N_{pix}\times F_{out}}$ of the last GC (or the output $\boldsymbol{Y}\in\mathbb{R}^{N_{stat}\times F_{out}}$ of the ST), ...
...
% The cost (or loss) function $C(\boldsymbol{Y},\bar{\boldsymbol{Y}}) = C(NN_\theta(\boldsymbol{X}),\bar{\boldsymbol{Y})$ measures how good the prediction $\boldsymbol{Y}$ is for sample $\boldsymbol{X}$, given the ground truth $\bar{\boldsymbol{Y}}$.
...
% The goal of learning is to find the parameters $\theta$ of the neural network that minimize the risk $R(\theta) = E\left[&#x10fc06;C\left(&#x10fc00;NN_\theta(\boldsymbol{X}),\bar{\boldsymbol{Y}}\right)\right]$&#x10fc01;&#x10fc07;.
